...
<html>
<head>
	<title>Laporan Pesanan Penjualan | PT MIRES MAHISA GLOBALINDO</title>
...
<body>
    ...
    <div style="width:100%; text-align: center; margin-bottom: 10px; margin-top: -20px"><strong style="font-size: 20px;">PESANAN PENJUALAN</strong> </div>
	...
	<table class="table table-bordered table-sm" style="width:100%">
		<thead>
			<tr class="text-center-row">
				...
				<th style="width:15%">Nomor SO</th>
				<th style="width:20%">Tanggal SO</th>
				<th style="width:20%">Jenis Penjualan</th>
				<th style="width:20%">Status Kirim</th>
				<th style="width:20%">Status Bayar</th>
			</tr>
		</thead>
		<tbody>
			@php $i=1 @endphp
			...
			<tr>
				...
				<td>{{ $r->so_nomer }}</td>
				<td>{{ date('d-m-Y', strtotime($r->so_tanggal)) }}</td>
                <td>{{ $r->jenis_penjualan }}</td>
                <td>
                    @if($r->status_do == 0)
                    Belum Dikirim
                    @elseif($r->status_do == 1)
                    Dikirim Sebagian
                    @elseif($r->status_do == 2)
                    Sudah Dikirim
                    @endif
                </td>
                <td>
