...
<html>
<head>
	<title>Laporan Pembayaran Pelanggan | PT MIRES MAHISA GLOBALINDO</title>
...
<body>
    ...
    <div style="width:100%; text-align: center; margin-bottom: 10px; margin-top: -20px"><strong style="font-size: 20px;">PEMBAYARAN PELANGGAN</strong> </div>
	...
	<table class="table table-bordered table-sm" style="width:100%">
		<thead>
			<tr class="text-center-row">
				...
				<th style="width:20%">Nomor CR</th>
				<th style="width:20%">Tanggal CR</th>
				<th style="width:30%">Pelanggan</th>
				<th style="width:25%">Jumlah yang Dibayar</th>
			</tr>
		</thead>
		<tbody>
			@php $i=1 @endphp
			...
			<tr>
				...
				<td>{{ $r->nomer_cr }}</td>
				<td>{{ date('d-m-Y', strtotime($r->tanggal_cr)) }}</td>
				<td>{{ $r->pelanggan->nama_pelanggan }}</td>
				<td>Rp. {{ number_format($r->total_payment, 0, ',', '.') }}</td>
			</tr>
			@endforeach
		</tbody>
	</table>
