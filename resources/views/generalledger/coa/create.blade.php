...
@section('header')
List Account (COA) (Daftar Akun) Baru
@endsection
            ...
            <div class="card-body">
                ...
                <select class="form-control" name="id_coatype" required>
                    ...
                    @foreach($tipeCoa as $row)
                        <option value="{{ $row->id }}" {{ $row->id == old('id_coatype') ? "selected" : "" }}>{{ $row->tipecoa }}</option>
                    @endforeach
                </select>
                <label>Nomer Akun <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="nomer_coa" value="{{ old('nomer_coa') }}" required>
               
                <label>Nama Akun <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="nama_coa" value="{{ old('nama_coa') }}" required>

                <label>Saldo Awal <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="saldo_awal" id="saldo_awal" value="{{ old('saldo_awal') }}" required>
            
                <label>Keterangan</label>
                <textarea class="form-control" rows="4" name="keterangan">{{ old('keterangan') }}</textarea>
