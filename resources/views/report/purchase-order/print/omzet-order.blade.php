...
<body class="A4" >
    <section class="sheet padding-10mm">
        <div class="text-center">
            ...
            <h2 style="text-transform: uppercase;color: #800000;margin-top:-10px" >Pembelian Per Biaya</h2>
            ...
            <table style="width: 100%;margin-top:20px" >
                <thead>
                    <tr>
                        ...
                        <th style="border:none;text-align: start" >Jumlah Tagihan</th>
                        <th style="border:none;text-align: start" >Jumlah Yang Dibayar</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td style="text-align: start" >{{ $item['vendor'] }}</td>
                            @php
                                $newDate = date_create($item['tanggal']);
                                $newDate = date_format($newDate, 'd M Y');
                            @endphp
                            <td style="text-align: start" >{{ $newDate }}</td>
                            <td style="text-align: start" >Rp. {{ number_format($item['jumlah_tagihan'], 0, ',', '.') }}</td>
                            <td style="text-align: start" >Rp. {{ number_format($item['jumlah_bayar'], 0, ',', '.') }} </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
