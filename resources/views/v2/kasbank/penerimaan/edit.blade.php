...
<a href="{{ route('kasbank.penerimaan.index') }}">
    <i class="fa fa-arrow-left"></i> Kembali ke daftar
</a>

<hr>

<small>Dibuat pada : {{$data['penerimaan']->created_at}}, oleh : {{ $data['penerimaan']->dibuatOleh->name }} </small>
<br>
<small>Terakhir diperbarui : {{$data['penerimaan']->updated_at}}, oleh : @if($data['penerimaan']->updated_by !== 0) {{ $data['penerimaan']->diupdateOleh->name }} @endif </small>
<form action="{{route('kasbank.penerimaan.update', $data['penerimaan']->id)}}" method="post" enctype="multipart/form-data">
    @csrf
    @method('patch')
    <!-- Data Penerimaan kasbank -->
    <div class="card">
            ...
            <div class="table-responsive">
                <table class="table table-borderless table-striped">
                    <tr>
                        ...
                        <td>
                            <select name="bank_id" class="form-control select2" required>
                                <option value="{{ $data['penerimaan']->bank_id }}">{{ $data['penerimaan']->bank->nomer_coa }} | {{ $data['penerimaan']->bank->nama_coa }}</option>
                    ...
                    <tr>
                        <td>Nomer*</td>
                        <td>
                            <input type="text" name="nomer" id="" class="form-control" value="{{ $data['penerimaan']->nomer }}" placeholder="Ex: BBK/1001" required>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td>Tanggal*</td>
                        <td>
                            <input type="date" name="tanggal" id="" class="form-control" value="{{ $data['penerimaan']->tanggal }}" required>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td>Keterangan</td>
                        <td>
                            <textarea name="keterangan" id="" cols="30" rows="5" class="form-control">{{ $data['penerimaan']->keterangan }}</textarea>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody id="rincian-akun">
                        @php $i = 0 @endphp
                        @foreach($data['penerimaan']->rincianAkun as $rincian)
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="col-md-6">
                <p class="text-danger">Support format berkas : <b>pdf | jpeg | jpg | png</b></p>
                @if($data['penerimaan']->rincianBerkas()->exists())
                <table class="table table-borderless">
                    @foreach($data['penerimaan']->rincianBerkas as $berkas)
                    <tr>
                        <td>Berkas</td>
                        <td>
                            <a href="{{ route('kasbank.penerimaan.download-berkas',$berkas->nama_berkas) }}">{{ $berkas->nama_berkas }}</a>
                        </td>
                    </tr>
                    @endforeach
                </table>
    ...
    <div class="card">
        <div class="card-footer">
            ...
            <a href="{{ route('kasbank.penerimaan.index') }}" class="btn btn-outline-warning">
                Batal
            </a>
...
<div class="modal fade" id="modalHapus" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <form style="width: 100%;" action="{{ route('kasbank.penerimaan.destroy',$data['penerimaan']->id) }}" method="post">
            @csrf
            @method('delete')
...
<script type="text/javascript">
    $(document).ready(function() {
        let row_akun = <?php echo $data['penerimaan']->rincianAkun()->count(); ?>;
        let row_berkas = 1;
