...
<form action="{{route('bukubesar.coa.update',$data['coa']->id)}}" method="post">
    @csrf
    @method('patch')
    <!-- Data Akun -->
    <div class="card">
        ...
        <div class="card-body">
            ...
            <select name="coa_tipe_id" class="form-control" required>
                <option value="{{ $data['coa']->coa_tipe_id }}">{{ $data['coa']->tipe->coa_tipe }}</option>
            ...
            <input type="text" name="nomer_coa" class="form-control" value="{{ $data['coa']->nomer_coa }}" required>

            <label for="">Nama</label>
            <input type="text" name="nama_coa" class="form-control" value="{{ $data['coa']->nama_coa }}" required>

            <label for="">Keterangan</label>
            <textarea name="keterangan" cols="30" rows="5" class="form-control">{{ $data['coa']->keterangan }}</textarea>
        </div>
    </div>
    <br>
