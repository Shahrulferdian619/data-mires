...
<div class="card">
    <div class="card-header pb-3">
        <h5 class="m-0 me-2 card-title">
            ...
            <a href="{{ route('pembelian.pesanan-pembelian.create') }}" class="btn btn-sm btn-outline-primary">
    ...
    <div class="card-datatable table-responsive">
        <table class="table table-hover" id="tabel_dt">
            ...
            <tbody>
                @foreach($data['pesananPembelian'] as $pesananPembelian)
                <tr style="background-color: @if($pesananPembelian->status_proses == 0) #fcd88b @elseif($pesananPembelian->status_proses == 1) #98fc8b @elseif($pesananPembelian->status_proses == 10) #ffa1b7 @endif;">
                    <td>
                        <a href="{{ route('pembelian.pesanan-pembelian.show',$pesananPembelian->id) }}">
                            {{$pesananPembelian->nomer_pesanan_pembelian}}
                        </a>
                    </td>
                    <td>
                        {{date('d/m/Y',strtotime($pesananPembelian->tanggal))}}
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        <small class="bg-label-primary">{{$rincian->item->nama_barang}} {{$rincian->deskripsi_item}}</small>,
                        @endforeach
                    </td>
                    <td>
                        {{$pesananPembelian->keterangan}}
                    </td>
                    <td style="text-align: right;">
                        {{ number_format($pesananPembelian->grandtotal,2) }}
                    </td>
                </tr>
                @endforeach
            </tbody>
