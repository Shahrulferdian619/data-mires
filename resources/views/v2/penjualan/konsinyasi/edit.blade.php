...
<form action="{{ route('konsinyasi.update',$konsinyasi->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('put')
    <!-- Header Konsinyasi -->
    <div class="card">
        ...
        <div class="card-body">
            <select name="pelanggan_id" class="form-control select2 pelanggan" required>
                <option value="{{ $konsinyasi->pelanggan_id }}">{{ $konsinyasi->pelanggan->kode_pelanggan }} | {{ $konsinyasi->pelanggan->nama_pelanggan }}</option>
            ...
            <input name="nama_pelanggan" type="text" class="form-control nama-pelanggan" value="{{ $konsinyasi->pelanggan->nama_pelanggan }}" style="background-color: #ebebeb;">

            <label for="">Alamat pelanggan</label>
            <textarea name="alamat_pelanggan" cols="30" rows="5" class="form-control alamat-pelanggan" style="background-color: #ebebeb;">{{ $konsinyasi->pelanggan->detil_alamat }} {{ $konsinyasi->pelanggan->kota }} {{ $konsinyasi->pelanggan->provinsi }}</textarea>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <select name="gudang_asal" class="form-control select2 gudang" required>
                <option value="{{ $konsinyasi->gudang_asal }}">{{ $konsinyasi->gudang_asal }}</option>
            ...
            <select name="gudang_tujuan" class="form-control select2 gudang" required>
                <option value="{{ $konsinyasi->gudang_tujuan }}">{{ $konsinyasi->gudang_tujuan }}</option>
                <option value="gudang-baru">-- GUDANG BARU --</option>
                @foreach($gudang as $g)
                <option value="{{ $g->nama_gudang }}">{{ $g->nama_gudang }}</option>
                @endforeach
            ...
            <input name="nomer_konsinyasi" type="text" class="form-control" value="{{ $konsinyasi->nomer_konsinyasi }}" required>

            <label for="">Tanggal konsinyasi</label>
            <input name="tanggal_konsinyasi" type="date" class="form-control" value="{{ $konsinyasi->tanggal_konsinyasi }}" required>

            <label for="">Keterangan</label>
            <textarea name="keterangan" cols="30" rows="5" class="form-control">{{ $konsinyasi->keterangan }}</textarea>
            ...
            <input name="penerima" type="text" class="form-control penerima" value="{{ $konsinyasi->penerima }}">

            <label for="">Alamat penerima</label>
            <textarea name="alamat_penerima" cols="30" rows="5" class="form-control alamat-penerima">{{ $konsinyasi->alamat_penerima }}</textarea>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody class="rincian-produk">
                        @foreach($konsinyasi->rinci as $rinci)
                        <tr>
                            <td>
                            ...
                            <td>
                                <input name="kuantitas[]" type="text" class="form-control kuantitas" value="{{ $rinci->kuantitas }}" required>
                            </td>
                            <td>
                                <input name="harga[]" type="text" class="form-control harga-produk" value="{{ $rinci->harga }}" required>
                            </td>
                            <td>
                                <input name="subtotal[]" type="text" class="form-control subtotal" value="{{ $rinci->subtotal }}" readonly hidden>
                                <input type="text" class="form-control konversi-subtotal" readonly>
                            </td>
                            <td>
                                <input name="catatan[]" type="text" class="form-control catatan" value="{{ $rinci->catatan }}">
                            </td>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <input type="text" class="form-control" value="{{ $konsinyasi->berkas->berkas1 }}" readonly>
        ...
        <form action="{{ route('api.store-pelanggan') }}" method="post" style="width: 100%;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input name="handphone_pelanggan" type="text" class="form-control">
                    ...
                    <textarea name="detail_alamat" cols="30" rows="5" class="form-control"></textarea>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">
                        Tutup
