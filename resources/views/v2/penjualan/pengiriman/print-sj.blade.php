...
<head>
    <title>Delivery Order : {{ $pengiriman_penjualan->nomer_pengiriman_penjualan }}</title>
    <meta charset="UTF-8">
    ...
    <div style="width:100%; padding:10px; border: 1px solid black">
        ...
        <table style="width: 100%; margin-top: 10px">
            <tr>
                ...
                <td>{{ $pengiriman_penjualan->pelanggan->nama_pelanggan }}</td>
                <td><strong>No. Surat Jalan</strong></td>
                <td>{{ $pengiriman_penjualan->nomer_pengiriman_penjualan }}</td>
            </tr>
            <tr>
                <td><strong>Alamat Customer</strong></td>
                <td>{{ $pengiriman_penjualan->pelanggan->detil_alamat }}</td>
                <td><strong>Tanggal Surat Jalan</strong></td>
                <td>{{ date('d-m-Y', strtotime($pengiriman_penjualan->created_at)) }}</td>
            </tr>
            <tr>
                <td><strong>No. Telp</strong></td>
                <td>{{ $pengiriman_penjualan->pelanggan->no_handphone }}</td>
                <td><strong>No. Sales Order</strong></td>
                <td>{{ $pengiriman_penjualan->pesanan->nomer_pesanan_penjualan }}</td>
            </tr>
            <tr>
                <td><strong>Ekspedisi</strong></td>
                <td>{{ $pengiriman_penjualan->ekspedisi }}</td>
                <td><strong>Tanggal Sales Order</strong></td>
                <td>{{ date('d-m-Y',strtotime($pengiriman_penjualan->tanggal)) }}</td>
            </tr>
            <tr>
                <td><strong>No. Resi</strong></td>
                <td>{{ $pengiriman_penjualan->resi }}</td>
                <td></td>
                <td></td>
            </tr>
        </table>
        ...
        <table style="width: 100%; margin-top: 10px">
            ...
            <tbody>
                ...
                @foreach($pengiriman_penjualan_rinci as $rinci)
                <tr>
        ...
        <table style="width: 100%; margin-top: 10px">
            <tr>
                ...
                <td style="vertical-align: text-top;" rowspan="2">
                    Note :
                    <p>{{ $pengiriman_penjualan->alamat_penerima }}</p>
                </td>
            </tr>
