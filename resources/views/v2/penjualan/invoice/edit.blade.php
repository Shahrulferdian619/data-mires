...
<form action="{{route('invoice-penjualan.update',$invoice->id)}}" method="post" enctype="multipart/form-data">
    @csrf
    @method('put')
    <!-- Data Pelanggan -->
    <div class="card">
        ...
        <div class="card-body">
            ...
            <select name="pelanggan_id" class="form-control select2 select-pelanggan" required>
                <option value="{{ $invoice->pelanggan_id }}">{{ $invoice->pelanggan->nama_pelanggan }}</option>
            ...
            <input type="text" class="form-control kode-pelanggan" value="{{ $invoice->pelanggan->kode_pelanggan }}" readonly>

            <label for="">Nama pelanggan</label>
            <input type="text" class="form-control nama-pelanggan" value="{{ $invoice->pelanggan->nama_pelanggan }}" readonly>

            <label for="">Alamat pelanggan</label>
            <textarea cols="30" rows="5" class="form-control alamat-pelanggan">{{ $invoice->pelanggan->detil_alamat }} {{ $invoice->pelanggan->provinsi }}</textarea>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <input name="nomer_invoice_penjualan" type="text" class="form-control" value="{{ $invoice->nomer_invoice_penjualan }}">

            <label>Jenis penjualan</label>
            <select name="jenis_penjualan" class="form-control select2" required>
                <option value="{{ $invoice->jenis_penjualan }}">{{ $invoice->jenis_penjualan }}</option>
                <option value="KONSINYASI">KONSINYASI</option>
                <option value="EVENT">EVENT</option>
            </select>

            <label for="">Nomer ref</label>
            <input name="nomer_ref" type="text" class="form-control" value="{{ $invoice->nomer_ref }}">

            <label for="">Tanggal</label>
            <input name="tanggal" type="date" class="form-control" value="{{ date('Y-m-d',strtotime($invoice->tanggal)) }}" required>

            <label for="">No. pesanan</label>
            <input name="nomer_pesanan" type="text" class="form-control" value="{{ $invoice->nomer_pesanan }}">

            <label for="">Sales</label>
            <select name="sales_id" class="form-control select2">
                <option value="{{ $invoice->sales_id }}">{{ $invoice->sales->nama_sales }}</option>
            ...
            <select name="ppn" class="form-control select-ppn" required>
                <option value="{{ $invoice->ppn }}">@if($invoice->ppn == 0) TIDAK @elseif($invoice->ppn == 1) YA @elseif($invoice->ppn == 2) INCLUDE PPn @endif</option>
            ...
            <textarea name="keterangan" cols="30" rows="5" class="form-control">{{ $invoice->keterangan }}</textarea>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <select name="gudang_id" class="form-control select2" required>
                <option value="{{ $invoice->gudang_id }}">{{ $invoice->gudang->nama_gudang }}</option>
                @foreach($gudang as $g)
                ...
                <table class="table table-bordered">
                    ...
                    <tbody class="rincian-produk">
                        ...
                        <tr>
                            ...
                            <td>
                                <input name="diskon_persen[]" type="number" step="0.01" class="form-control diskon-persen" value="{{ $rinci->diskon_persen }}">
                            </td>

                            <td>
                                <input name="diskon_nominal[]" type="number" class="form-control diskon-nominal" value="{{ $rinci->diskon_nominal }}">
                            </td>

                            <td>
                                <input name="subtotal[]" type="text" class="form-control subtotal" value="{{ $rinci->subtotal }}" readonly style="background-color: #ebebeb;">
                            </td>

                            <td>
                                <textarea name="catatan[]" class="form-control">{{ $rinci->catatan }}</textarea>
                            </td>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="akun_diskon_id" id="akun_diskon" class="form-control select2" required>
                                        <option value="{{ $invoice->akun_diskon_id }}">{{ $invoice->akun_diskon->nama_coa }}</option>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="akun_ppn_id" id="akun_ppn" class="form-control select2" required>
                                        <option value="{{ $invoice->akun_ppn_id }}">{{ $invoice->akun_ppn->nama_coa }}</option>
    ...
    $(document).ready(function() {
        ...
        $btnTambahProduk.click(function() {
            const rowProduk = `<tr>
                                    ...
                                    <td>
                                        <input name="diskon_persen[]" type="number" step="0.01" class="form-control diskon-persen" value="0">
                                    </td>
                ...
                success: function(data) {
                    for (i = 0; i < data.jumlah; i++) {
                        let rowPaket = `<tr>
                                            ...
                                            <td style="background: #ebebeb;">
                                                <input name="diskon_persen[]" type="number" class="form-control step="0.01" diskon-persen" value="${dsc_paket}">
                                            </td>
                                            <td style="background: #ebebeb;">
                                                <input name="diskon_nominal[]" type="text" class="form-control diskon-nominal" value="0">
                                            </td>
