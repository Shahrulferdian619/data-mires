...
<form action="{{route('permintaan-tester.update',$tester->id)}}" method="post" enctype="multipart/form-data">
    @csrf
    @method('put')
    <!-- Data Pelanggan -->
    <div class="card">
        ...
        <div class="card-body">
            ...
            <select name="pelanggan_id" class="form-control select2 pelanggan" required>
                <option value="{{ $tester->pelanggan_id }}">{{ $tester->pelanggan->nama_pelanggan }}</option>
            ...
            <input id="kode_pelanggan" value="{{ $tester->pelanggan->kode_pelanggan }}" type="text" class="form-control" readonly>

            <label for="">Nama pemesan</label>
            <input name="nama_pelanggan" value="{{ $tester->pelanggan->nama_pelanggan }}" type="text" class="form-control" readonly>

            <label for="">Alamat pemesan</label>
            <textarea name="alamat_pelanggan" cols="30" rows="5" class="form-control" readonly>{{ $tester->pelanggan->detil_alamat }} {{ $tester->pelanggan->provinsi }}</textarea>
            ...
            <input name="penerima" value="{{ $tester->penerima }}" type="text" class="form-control penerima" required>

            <label for="">Alamat penerima</label>
            <textarea name="alamat_penerima" cols="30" rows="5" class="form-control alamat-penerima" required>{{ $tester->alamat_penerima }}</textarea>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <input name="nomer_permintaan_tester" type="text" class="form-control" value="{{ $tester->nomer_permintaan_tester }}" readonly>

            <label for="">Tanggal</label>
            <input name="tanggal" type="date" class="form-control" value="{{ $tester->tanggal }}" required>

            <label for="">No. pesanan</label>
            <input name="nomer_pesanan" type="text" value="{{ $tester->nomer_pesanan }}" class="form-control">

            <label for="">Sales</label>
            <select name="sales_id" class="form-control select2">
                <option value="{{ $tester->sales_id }}">{{ $tester->sales->nama_sales }}</option>
            ...
            <select name="ekspedisi" class="form-control select2">
                <option value="{{ $tester->ekspedisi }}">{{ $tester->ekspedisi }}</option>
            ...
            <input name="resi" type="text" value="{{ $tester->resi }}" class="form-control">

            <label for="">Keterangan</label>
            <textarea name="keterangan" cols="30" rows="5" class="form-control">{{ $tester->keterangan }}</textarea>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <select name="gudang_id" class="form-control select2" required>
                <option value="{{ $tester->gudang_id }}">{{ $tester->gudang->nama_gudang }}</option>
                @foreach($gudang as $g)
                        ...
                        <tr>
                            <td>
                                <select name="produk_id[]" class="form-control select2 selectProduk" required>
                                    <option value="{{ $rincian->produk_id }}">{{ $rincian->produk->kode_barang }} | {{ $rincian->produk->nama_barang }}</option>
                            ...
                            <td>
                                <input name="kuantitas[]" type="number" class="form-control kuantitas" value="{{ $rincian->kuantitas }}" required>
                            </td>

                            <td>
                                <input name="catatan[]" type="text" class="form-control" value="{{ $rincian->catatan }}">
                            </td>
