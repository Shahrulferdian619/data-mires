                    ...
                    <div class="card-body">
                        ...
                        <select class="select2 form-control form-control-lg" name="kredit_coa_id" required>
                            ...
                            @foreach($coaKredit as $row)
                                <option value="{{ $row->id }}" {{ $row->id == $kredit->coa_id ? "selected" : "" }}>[{{ $row->nomer_coa }}] - {{ $row->nama_coa }}</option>
                            @endforeach
                        </select>
                        <label>Nomor <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" name="nomer" value="{{ $bukuBank->nomer }}" required>
                                ...
                                <tbody class="flag_row">
                                    @foreach ($bukuBankRinci as $item)
                                        <tr>
                                            <td>
                                                ...
                                                <select class="selectNya form-control akunNya" name="debit_coa_id[]" required>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" name="memo[]" id="memo_{{ $item->id }}" value="{{ $item->memo }}" placeholder="Masukan Memo...">
                                            </td>
                                            <td>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <select class="selectNya form-control akunNya" name="debit_coa_id[]">
                                                <option value="">-- PILIH AKUN --</option>
                                                @foreach($coaDebit as $row)
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="{{ route('admin.pembayaran.index') }}" class="btn btn-outline-secondary">Batal</a>
                    </div>
                </div>
            </div>
        </div>  
...
<script type="text/javascript">
    ...
    $('#batalSimpan').on('click', function() {
        ...
        if(countNominal == 1 && $('.nominal').eq(0).val() == "" && $('.akunNya').eq(0).val() == "" )
        {
            ...
            location.href = "{{ route('admin.pembayaran.index') }}";
        }
    ...
    $('body').on('submit', '#form-save', function(e) {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('admin.pembayaran.update', ['pembayaran' => $bukuBank->id]) }}",
            data: formData,
            processData: false,
            contentType: false,
            cache: false,
