...
<div class="card">
    <div class="card-body">
        ...
        <a href="{{ url('admin/penerimaan/'.$bukuBank->id.'/print') }}" class="btn btn-outline-success" type="submit">Print</a>
        <form action="{{ route('admin.penerimaan.destroy', ['penerimaan' => $bukuBank->id]) }}" method="POST" class="d-inline-block">
            @csrf
            @method('DELETE')
            <button onclick="return confirm('Yakin Akan Menghapus?')" class="btn btn-outline-danger" type="submit">Hapus</button>
        </form>
        <form action="{{ url('admin/penerimaan/export-excel/' . $bukuBank->id) }}" method="POST" class="d-inline-block">
            @csrf
            <button class="btn btn-outline-success" type="submit">Export Excel</button>
        </form>
        <form action="{{ url('admin/penerimaan/export-pdf/' . $bukuBank->id) }}" method="POST" class="d-inline-block">
            @csrf
